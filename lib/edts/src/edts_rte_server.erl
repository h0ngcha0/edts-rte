...
handle_cast(exit, #rte_state{} = State) ->
  AllReplacedFuns = generate_replaced_funs(State#rte_state.mfa_info_list) ++
                    State#rte_state.replaced_funs,
  send_function_to_clients(concat_replaced_funs(AllReplacedFuns)),
  {noreply, State};
...
send_function_to_clients(FunBody) ->
  edts_rte:debug("final function body is:~p~n", [FunBody]),
  lists:foreach( fun(Fun) -> Fun(FunBody) end
               , [ fun send_fun_to_rte_web/1
                 , fun send_fun_to_emacs/1
                 ]).
